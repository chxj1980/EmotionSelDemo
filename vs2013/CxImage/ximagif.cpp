...
bool CxImageGIF::Decode(CxFile *fp)
{
		...
		if (bPreviousWasNull || ch==0)
		{
			...
			case ',': // image
				...
				*/
				...
				if (!info.bGetAllFrames && info.nFrame==iImage) bContinue=false; else iImage++;
	...
	//如果是全部加载，则令 info.pImage(DIB data) 指向第一帧 -- hoodlum1980
	//加载完毕后，通常nFrame为0
	if(info.bGetAllFrames && info.nFrame>=0 && info.nFrame < info.nNumFrames -1)
	{
		Copy(*ppFrames[info.nFrame], true, true, true);
	}
	return true;
}
////////////////////////////////////////////////////////////////////////////////
